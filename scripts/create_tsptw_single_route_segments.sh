...
DROP TABLE IF EXISTS tsptw_single_route_segments;

CREATE TABLE tsptw_single_route_segments AS
SELECT
  a.stop_order,
...
FROM tsptw_single_route a
JOIN tsptw_single_route b
  ON b.stop_order = a.stop_order + 1
...
CREATE INDEX tsptw_single_route_segments_geog_idx ON tsptw_single_route_segments USING gist(the_geog);
